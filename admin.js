...
const openEditModal = (tableName, rowId) => {
    ...
    editFields.innerHTML = fields.map(field => {
        ...
        if (field.type === 'textarea') {
            return `
                <label class="${field.fullWidth ? 'md:col-span-2' : ''} block">
                    ...
                    <textarea name="${field.key}" rows="4" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:outline-none focus:border-primary-500">${safeValue}</textarea>
                </label>
            `;
        }
        ...
        return `
            <label class="${field.fullWidth ? 'md:col-span-2' : ''} block">
                <span class="block text-sm font-medium mb-2 text-gray-300">${field.label}</span>
                <input name="${field.key}" type="${field.type}" value="${safeValue}" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 focus:outline-none focus:border-primary-500">
            </label>
        `;
    }).join('');
